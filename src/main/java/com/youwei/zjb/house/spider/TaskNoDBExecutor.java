...
public class TaskNoDBExecutor extends Thread{
	private Task task;
	
	public TaskNoDBExecutor(Task task){
		this.task = task;
	}
...
//		System.out.println(pageHtml);
		if(dataList.isEmpty()){
			...
			LogUtil.info(task.name+"....."+task.lastError);
			return;
		}
		for(int i=dataList.size()-1;i>=0;i--){
			...
			try {
				...
				LogUtil.info(detailUrl+"信息不完整:"+ex.getMessage());
				continue;
			}catch(Exception ex){
    ...
    private void write(String input)  {
           File f = new File("house_"+new Date().getTime());
	...
	private void prcessChuzu(String detailUrl) throws IOException, NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, DataInvalidException {
		...
		hr.peizhi = TaskHelper.getPeiZhiFromText(peizhi);//配置
		hr.title = getDataBySelector(page , "cuzuTitle");//标签
